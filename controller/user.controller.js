...
class UserController {
                ...
                for (let index = 0; index < user.length; index++) {
                    tBody += `<tr>
                                    ...
                                    <input hidden name="id" value="${user[index].id}" >
                                     <a href="/user/delete/${user[index].id}" class=" btn btn-danger" onclick="return confirm('Bạn có thực sự muốn xóa? ')"> Delete</a>
                                     </td>
                                 </tr>`
                }
                data = data.replace('{users}', tBody);
    ...
    showTable(req,res) {
        fs.readFile('teamplate/tables.html', 'utf-8', async (err, data) => {
            if (err) {
                console.log('file is not')
            } else {
                let id = req.params.id;
